...
function FilterItem({ title, values, id, prices, filterDataArray, setFilterDataArray }) {
    ...
    function handleFilter(valueId, value, toggle) {
        ...
        if (toggle) {
            ...
            if(!filterDataArray.filter(el => el[id] !== value).length) {
                ...
                dispatch(fetchFilterCategory({ filterData: [...filterDataArray.filter(el => el[id] !== value), {9999: [...prices]}], categoryId: singleCategoryData.id, limit: 20 }))
            }
        } else {
            setFilterDataArray([
            ...
            dispatch(fetchFilterCategory({ filterData: [...filterDataArray, { [id]: value }, {9999: [...prices]}], categoryId: singleCategoryData.id, limit: 20 }))
        }
    }
                            ...
                            values.map(value => (
                                <li key={value.id}>
                                    <label className="checkbox">
                                        <input type="checkbox" onClick={() => { handleFilter(value.id, value.value, value.toggle) }} />
